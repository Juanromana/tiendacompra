...
public class ControladorCliente implements ActionListener {
    ...
    public void iniciarCliente() {
        ...
        cli.getJcbsexo().addItem("Seleccionar:");
        Map<String, Integer> dato = modcli.llenarCombo("sexo");
        for (String sexo : dato.keySet()) {
            cli.getJcbsexo().addItem(sexo);
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == (cli.getBtnGuardarCliente())) {
            ...
            if ((cli.getCbxtipodedocu().getSelectedItem().equals("Seleccionar:")) 
                    || (cli.getTxtDocumento().getText().isEmpty()) 
                    || (cli.getJcbsexo().getSelectedItem().equals("Seleccionar:"))
                    || (cli.getTxtNombre().getText().isEmpty()) 
                    || (cli.getTxtTelefono().getText().isEmpty()) 
                    || (cli.getTxtCorreo().getText().isEmpty()) 
                    || (cli.getTxtDireccion().getText().isEmpty()) 
                    || (cli.getJDFechadeNaci().getDate() == null)) {
                JOptionPane.showMessageDialog(null, "Hace Falta Informacion");
            } else {
                JOptionPane.showMessageDialog(null, "registro almacenado");
                String valorSexo = cli.getJcbsexo().getSelectedItem().toString();
                int sexo = modcli.llenarCombo("sexo").get(valorSexo);
                java.util.Date fec = cli.getJDFechadeNaci().getDate();
                long fe = fec.getTime();
                ...
                modcli.setSex(sexo);
                modcli.setTipodedocumento(cli.getCbxtipodedocu().getSelectedItem().toString());
    ...
    public void actualizarCliente(int Doc) {
        ...
        cli.getJDFechadeNaci().setDate(modcli.getFec());
        cli.getCbxtipodedocu().setSelectedItem(modcli.getTipodedocumento());
        ...
        cli.getJcbsexo().setSelectedItem(valorSexo);

        prin.setVisible(false);
        cli.getLblCliente().setText("ACTUALIZAR CLIENTE");
        cli.setLocationRelativeTo(null);
